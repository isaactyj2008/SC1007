...
void interleaveQueue(Queue *q)
{
  ...
  // Push the first half of the queue onto the temporary stack
  for (int i = 0; i < half_size; i++) {
      push(&temp, dequeue(q));
  }

  // Interleave elements from the stack back into the queue
  while (!isEmptyStack(&temp)) {
      enqueue(q, pop(&temp));
  }

  // Move the elements from the front of the original queue to the rear
  for (int i = 0; i < half_size; i++) {
      enqueue(q, dequeue(q));
  }

  // Interleave elements from the rear of the original queue into the front
  for (int i = 0; i < half_size; i++) {
      push(&temp, dequeue(q));
  }
